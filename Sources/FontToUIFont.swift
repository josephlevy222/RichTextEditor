...
extension UIFont {
	...
	public class func preferredFontStyle(from: Font.TextStyle) -> UIFont.TextStyle  {
		let uiStyleOfFontStyle : [Font.TextStyle : UIFont.TextStyle] = [
			...
			.headline : .headline, 		.callout : .callout, 	.caption : .caption1, 	.caption2 : .caption2,
			.footnote : .footnote,		.body : .body ]
		return uiStyleOfFontStyle[from] ?? .body
	}
}

public func systemDesign(design: Font.Design) -> UIFontDescriptor.SystemDesign {
	let systemDesigns : [Font.Design : UIFontDescriptor.SystemDesign] =
	[.default : .default, 	.monospaced : .monospaced, 	.rounded : .rounded, 	.serif : .serif]
	return systemDesigns[design] ?? .default
}
...
struct SystemProvider: FontProvider {
	...
	func fontDescriptor(with traitCollection: UITraitCollection?) -> UIFontDescriptor {
		var descriptor = UIFont.preferredFont(forTextStyle: .body, compatibleWith: traitCollection).fontDescriptor
		descriptor = descriptor.withDesign(design) ?? descriptor
		return descriptor.withWeight(weight).addingAttributes([.size: size])
	}
}
...
func resolveFontProvider(_ provider: Any) -> FontProvider? {
	...
	case "PlatformFontProvider":
		guard let base = mirror.descendant("font") as? UIFont else { break }
		return PlatformFontProvider(uiFont: base)
		
	case "StaticModifierProvider<ItalicModifier>":
		guard let base = mirror.descendant("base", "provider", "base") else { break }
	...
	case "StaticModifierProvider<MonospacedModifier>":
		guard let base = mirror.descendant("base", "provider", "base") else { break }
		return resolveFontProvider(base).map(StaticModifierProvider<MonospacedModifier>.init)
		
	case "StaticModifierProvider<MonospacedDigitModifier>":
		guard let base = mirror.descendant("base", "provider", "base") else { break }
		return resolveFontProvider(base).map(StaticModifierProvider<MonospacedDigitModifier>.init)
		
	case "SystemProvider":
		guard let size = mirror.descendant("size") as? CGFloat else { break }
		let design = systemDesign(design: mirror.descendant("design") as? Font.Design ?? .default)
		let weight = UIFont.Weight(rawValue:(mirror.descendant("weight","some","value") ?? 0) as? CGFloat ?? 0)
		return SystemProvider(size: size, design: design, weight: weight)
		
	case "NamedProvider":
		guard let name = mirror.descendant("name") as? String,
			  let size = mirror.descendant("size") as? CGFloat else { break }
		let textStyle = mirror.descendant("textStyle") as? Font.TextStyle
		let uiStyle = UIFont.preferredFontStyle(from: textStyle ?? .body)
		return NamedProvider(name: name, size: size, textStyle: uiStyle)
		
	case "TextStyleProvider":
		guard let style = mirror.descendant("style") as? Font.TextStyle else { break }
		let design = mirror.descendant("design") as? Font.Design ?? Font.Design.default
		let weight = mirror.descendant("weight") as? UIFont.Weight
		let uiStyle = UIFont.preferredFontStyle(from: style)
		return TextStyleProvider(style: uiStyle, design: systemDesign(design: design) , weight: weight)
		
	case "ModifierProvider<WeightModifier>":
		guard let base = mirror.descendant("base", "provider", "base"),
			  let weight = mirror.descendant("modifier", "weight", "value")  else { break }
	...
	case "ModifierProvider<WidthModifier>":
		guard let base = mirror.descendant("base", "provider", "base"),
			  let width = mirror.descendant("modifier", "width")  else { break }
